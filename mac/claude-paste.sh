#!/bin/bash
# =============================================================================
# claude-paste.sh - Screenshot and upload to remote server
# =============================================================================
# Usage:
#   1. Have the terminal tab active in iTerm2
#   2. Press Cmd+Shift+V (or run this script directly)
#   3. Select area to screenshot with crosshair
#   4. Path auto-pastes into the terminal - just add a question!
#
# Prerequisites (install on the Mac):
#   brew install pngpaste
#
# Configuration:
#   - Set SSH_ALIAS to the SSH config alias or user@host
...
# =============================================================================
# CONFIGURATION - Edit these for the setup
# =============================================================================
SSH_ALIAS="${CLAUDE_SSH_ALIAS:-server-alias}"       # SSH alias or user@host
REMOTE_PATH=".claude/paste-cache/images"            # Remote directory for images
REMOTE_HOME_FALLBACK="/home/username"               # Remote home directory
...
if ! command -v pngpaste &>/dev/null; then
    osascript -e 'display notification "pngpaste not installed. Run: brew install pngpaste" with title "Screenshot Paste" sound name "Basso"'
    exit 1
fi
...
if ! pngpaste "$TEMP_FILE" 2>/dev/null; then
    osascript -e 'display notification "Screenshot cancelled or failed" with title "Screenshot Paste" sound name "Basso"'
    exit 1
fi
...
if scp -q "$TEMP_FILE" "${SSH_ALIAS}:${REMOTE_PATH}/${FILENAME}" 2>/dev/null; then
    ...
    osascript -e 'display notification "Upload failed - check SSH connection" with title "Screenshot Paste" sound name "Basso"'
    exit 1
fi
